...
{
    // Convert Clucoin:// to Clucoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Clucoin://"))
    {
        uri.replace(0, 12, "Clucoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Clucoin.lnk";
}
...
{
    return GetAutostartDir() / "Clucoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Clucoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Clucoin-qt [" + tr("command-line options") + "]                     " + "\n";
